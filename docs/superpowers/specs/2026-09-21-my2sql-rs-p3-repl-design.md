...
- **不重写已存在文件**：无启动一次性预检；闸在**首个冲突目标的创建时刻**原子生效——写文件走 `create_new`（O_EXCL），盘上已存在同名文件即该次创建失败 → 硬错并报出该冲突名（换 `--output-dir` 后重试由用户决定），逐次一个、race 安全（预检式方案在 TOCTOU 窗口下不可保证，实现口径以 O_EXCL 为准）。**repl 永不 append 既有文件**——接续产物永远新文件，旧产物字节不可变，这是 §4 at-least-once 语义能被人手消费的前提。
...
- **自动重连**：指数退避 1s → 30s 封顶 + 抖动，**无限次**；每次重连日志一行（`repl: reconnect #K in <backoff> at <file:pos>`），同一波故障日志限流。终止条件（不可恢复，立即非零退出并给可操作信息）：认证失败（1045）、缺 `REPLICATION SLAVE/CLIENT` 权限（1227 家族）、位点被 purge（1236）、server-id 冲突特征（表现为对端强制断连循环 → 连续 3 次同因秒断即终止报错，防无限互踢；**秒断 = 零进度**——开流成功且 0 事件投递才计连发，带进度的串中断路径与重启窗口的开流被拒一律清零）。主库重启（container restart 级）走正常重连恢复。
