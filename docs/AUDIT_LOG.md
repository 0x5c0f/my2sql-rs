...
## Findings (2026-09-24 — 三次审计：G1 变红，C1/C2 依证据下调)

**被审计树：** HEAD `978854b`（2026-09-24 13:56:16 +0800），`git status` 干净（仅未跟踪 `RTEST_GUIDE.md`）。
与同一日二次审计的差别：那一轮报告自述工作区处于未提交状态，两轮对象不是同一棵树；
模式与角色映射结论沿用（单成员 workspace，无 role crate）。

### New Findings

#### G1 — 格式化门禁失败
- **Status:** NEW（High）
- **Location:** `tests/e2e.rs:611`（82 字符）、`tests/e2e.rs:643`（92 字符），超 rustfmt 默认 `array_width = 60`
- **Evidence:** `cargo fmt --check` 退出 1，`Diff in` 头号 608 / 640；其余文件无 diff
- **Follow-up:** 二次审计时 G1 为 pass；`git blame` 显示两行由 HEAD 提交 `978854b` 引入（相关性，非已证实的因果）
- **Note:** 项目自身 CI（`.github/workflows/ci.yml`）以此为首个门禁步骤，会整体变红

### Recurring Findings（严重度依证据变化，非因重复而升级/降级）

#### C1 — 生产代码 `unwrap()`
- **Status:** RECURRING（High → Medium，本轮回调）
- **Location:** `src/flashback/reverse.rs:96,105,118`
- **Reclassification evidence:** ① `reverse.rs:110-116` 显式 `h.join()` 收集 `is_err()`，工作线程 panic 会转为 `Err` 上抛，gotcha「panic 被吞、任务静默死亡」的前提不成立；② 三处锁临界区仅 `pop_front()`/赋值，无 panic 路径，锁污染当前不可达；③ 上一轮记为 High 的 `src/pipeline/mod.rs:160` 经回读确认有前置守卫（同函数 `:138-142`），本轮剔除为误报
- **Residual:** 「锁从不跨 panic 路径持有」不变量未文档化

#### C2 — 运行期 `expect()`
- **Status:** RECURRING（High → Medium，本轮回调）
- **Location:** `src/pipeline/mod.rs:588,591,595`、`src/repl/assembly.rs:613`、`src/output.rs:298`
- **Reclassification evidence:** 均为同函数内可自证不变量（前置判空 / `ensure_sink(&key)?` 成功 / match 非重连臂已 break / `ckpt_out.is_some()` 前置返回），非环境依赖条件；其余 7 处不可失败或非生产

#### G5 — 3 条 CI 构建命令未本地验证
- **Status:** RECURRING（Medium，同严重度复报）
- **Location:** musl release build、`cargo check --manifest-path fuzz/Cargo.toml`、release 双目标矩阵 build

#### S3 — 测试内 `format!()` SQL
- **Status:** RECURRING（维持降级 High→Low）
- **Location:** `tests/repl.rs`（7）、`tests/e2e_drop_recovery.rs:274`、`src/pipeline/mod.rs:1362`（`#[cfg(test)]` 内）
- **Note:** 9 处全在测试代码，插值为测试本地常量；生产无 SQL 拼接

#### C9 — `println!`/`eprintln!`
- **Status:** RECURRING（维持降级 Medium→Low）
- **Location:** 13 处（`src/main.rs`、`src/config.rs:332`、`src/pipeline/mod.rs:154,652`、benches、fuzz）
- **Note:** CLI stdout/stderr 即产品输出契约（「P1 逐字节不变」）

#### C5 — `main.rs` 46 行
- **Status:** RECURRING（维持降级 High→Low）
- **Location:** `src/main.rs`
- **Note:** 约 26 行为设计注释，实际约 20 行纯装配，无业务逻辑

### Resolved / Not Applicable

- **MIGRATE-1：** 不适用（`[workspace]` 存在）——与二次审计一致
- **S5：** 跳过（`cargo-audit` 未安装）——连续三轮未验证，属未验证而非通过
...
| C10 | 无 TODO/FIXME/HACK | `src/` 生产路径零命中（唯一 `XXXX` 在 `#[cfg(test)] mod tests`） |
...
- **`pipeline/mod.rs:160` 误报（已剔除）：** 上一轮把 `cfg.output_dir.clone().unwrap()` 记为「无前置校验」的 High 生产 panic 路径，
  实际同函数 `src/pipeline/mod.rs:138-142` 先判 `output_dir.is_none()` 并返回 `PipelineError::Config`。
  教训：C1/C2 的模式匹配命中必须回读**同一函数**的守卫，不能只看命中行前后几行；跨行数超过扫描上下文窗口的守卫会被漏读。
- **`lock().unwrap()` 在 `std::thread::spawn` 内不一定符合「panic 被吞」gotcha：** 若调用方显式 `h.join()` 并收集 `is_err()`，
