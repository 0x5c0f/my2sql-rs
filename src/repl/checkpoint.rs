...
pub fn write_atomic(path: &Path, cp: &Checkpoint) -> io::Result<()> {
    ...
    // tmp 名 = `.{file}.tmp`（隐藏档，与正式档同目录，保证 rename 原子性）
...
/// 读档并交叉验证：JSON 合法后，`written_files` 与 `dir` 下实物**一一对应**
/// ——登记了但盘上缺 → `Missing`（硬错）；盘上有但未登记（checkpoint 档自身
/// 除外）→ `Stale`（硬错）。两者消息均含文件名，供运维定位清场范围。
